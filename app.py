...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS withdraw_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, user_1xbet_id TEXT, withdraw_code TEXT, amount INTEGER, qr_photo TEXT, status TEXT, date TEXT, time TEXT)''')
...
🏙 Шаар: Бишкек
🏢 Көчө: One More Dep (24/7)

📍 5. Тастыктоо
📍 6. Кодду алуу!
📍 7. Аны бизге ботко жөнөтүңүз

⚠️ Эгерде көйгөйлөр пайда болсо 👇

💻 Оператор: {SUPPORT}

💳 Колдонмонун/сайттын чыгуу кодун киргизиңиз:"""
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add("🔙 Назад")
...
def get_withdraw_code(msg):
    if msg.text == "🔙 Назад":
        ...
        return
    user_id = msg.chat.id
    temp_data[user_id] = {"withdraw_code": msg.text}
    bot.send_message(msg.chat.id, "🆔 1xBet IDңизди киргизиңиз:", reply_markup=back_menu())
...
def get_withdraw_id(msg):
    if msg.text == "🔙 Назад":
        ...
        return
    user_id = msg.chat.id
    temp_data[user_id]["withdraw_id"] = msg.text
    bot.send_message(msg.chat.id, "📸 QR кодду жибериңиз:", reply_markup=back_menu())
    bot.register_next_step_handler(msg, get_withdraw_qr)

def get_withdraw_qr(msg):
    if msg.text == "🔙 Назад":
        start(msg)
        return
    if not msg.photo:
        ...
        return
    user_id = msg.chat.id
    temp_data[user_id]["withdraw_qr"] = msg.photo[-1].file_id
    bot.send_message(msg.chat.id, "💰 Сумманы киргизиңиз (150дөн 50000ге чейин):", reply_markup=back_menu())
...
def get_withdraw_amount(msg):
    if msg.text == "🔙 Назад":
        ...
        return
    if not msg.text.isdigit():
        bot.send_message(msg.chat.id, "❌ Санды киргизиңиз!", reply_markup=back_menu())
        bot.register_next_step_handler(msg, get_withdraw_amount)
        return
    amount = int(msg.text)
    if amount < 150 or amount > 50000:
        bot.send_message(msg.chat.id, "❌ Сумма 150дөн 50 000 сомго чейин!", reply_markup=back_menu())
        bot.register_next_step_handler(msg, get_withdraw_amount)
        return
    user_id = msg.chat.id
    withdraw_code = temp_data[user_id].get("withdraw_code")
    user_1xbet_id = temp_data[user_id].get("withdraw_id")
    qr_photo = temp_data[user_id].get("withdraw_qr")
    if not withdraw_code or not user_1xbet_id or not qr_photo:
        bot.send_message(msg.chat.id, "❌ Ката! Башынан баштаңыз.")
        start(msg)
        return
    conn = sqlite3.connect('xgromkassa.db')
    c = conn.cursor()
    c.execute('INSERT INTO withdraw_requests (user_id, user_1xbet_id, withdraw_code, amount, qr_photo, status, date, time) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
              (user_id, user_1xbet_id, withdraw_code, amount, qr_photo, 'pending', datetime.now().strftime("%d.%m.%Y"), datetime.now().strftime("%H:%M:%S")))
    conn.commit()
    conn.close()
    admins = get_admins()
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("✅ Одобрить вывод", callback_data=f"approve_withdraw_{req_id}"), types.InlineKeyboardButton("❌ Отклонить вывод", callback_data=f"reject_withdraw_{req_id}"))
    for admin in admins:
        try:
            bot.send_photo(admin, qr_photo, caption=f"🆕 ЗАЯВКА НА ВЫВОД\n👤 {user_id}\n🆔 {user_1xbet_id}\n🔢 {withdraw_code}\n💰 {amount} сом", reply_markup=markup)
        except:
            pass
    bot.send_message(msg.chat.id, f"✅ Заявка на вывод принята!\n🆔 {user_1xbet_id}\n💰 {amount} сом", reply_markup=main_menu(user_id))
    del temp_data[user_id]
...
def view_withdraw_requests(msg):
    conn = sqlite3.connect('xgromkassa.db')
    c = conn.cursor()
    c.execute('SELECT id, user_id, user_1xbet_id, withdraw_code, amount, qr_photo, date, time FROM withdraw_requests WHERE status = "pending"')
    rows = c.fetchall()
    conn.close()
    if not rows:
        bot.send_message(msg.chat.id, "📭 Нет заявок на вывод")
        return
    for row in rows:
        req_id, user_id, user_1xbet_id, withdraw_code, amount, qr_photo, date, time = row
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("✅ Одобрить вывод", callback_data=f"approve_withdraw_{req_id}"), types.InlineKeyboardButton("❌ Отклонить вывод", callback_data=f"reject_withdraw_{req_id}"))
        text = f"💸 ЗАЯВКА НА ВЫВОД #{req_id}\n👤 {user_id}\n🆔 {user_1xbet_id}\n🔢 {withdraw_code}\n💰 {amount} сом"
        try:
            bot.send_photo(msg.chat.id, qr_photo, caption=text, reply_markup=markup)
        except:
            bot.send_message(msg.chat.id, text, reply_markup=markup)
...
def handle_call(call):
    ...
    if data.startswith('approve_'):
        ...
        conn = sqlite3.connect('xgromkassa.db')
        c = conn.cursor()
        c.execute('SELECT user_id, user_1xbet_id, amount FROM withdraw_requests WHERE id = ?', (req_id,))
        result = c.fetchone()
        conn.close()
        if result:
            user_id, user_1xbet_id, amount = result
            bot.answer_callback_query(call.id, "✅ Вывод одобрен!")
            try:
                bot.send_message(user_id, f"✅ ВЫВОД {amount} сом ОДОБРЕН!\n🆔 {user_1xbet_id}")
            except:
                pass
            bot.edit_message_text(f"✅ ЗАЯВКА #{req_id} ОДОБРЕНА", call.message.chat.id, call.message.message_id)
