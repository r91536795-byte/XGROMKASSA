...
# ID ФОТО ДЛЯ ПОПОЛНЕНИЯ
DEPOSIT_PHOTO_ID = "AgACAgIAAxkBAAEppgJqFVf8RJMDPdP8jXh2fVjJKX7tggACbR1rG7aLsEiMxUtKkOufhgEAAwIAA3gAAzsE"
...
def deposit(msg):
    ...
    bot.send_photo(msg.chat.id, DEPOSIT_PHOTO_ID, caption="🆔 Введите ваш **1xBet ID**:", parse_mode='Markdown', reply_markup=back_menu())
    bot.register_next_step_handler(msg, get_1xbet_id)

def get_1xbet_id(msg):
    if msg.text == "🔙 Назад":
        start(msg)
        return
    user_id = msg.chat.id
    temp_data[user_id] = {"1xbet_id": msg.text}
...
def get_check_photo(msg):
    ...
    user_1xbet_id = temp_data.get(user_id, {}).get("1xbet_id")
    amount = temp_data.get(user_id, {}).get("amount")
    photo_id = msg.photo[-1].file_id
    if not user_1xbet_id or not amount:
        bot.send_message(msg.chat.id, "❌ Ошибка! Начните заново.")
        start(msg)
        return
    dep_id = add_deposit(user_id, amount, user_1xbet_id, photo_id)
    admins = get_admins()
...
2️⃣ Назовите оператору:
   • Ваш 1xBet ID
   • Сумму вывода

3️⃣ Получите КОД от оператора

4️⃣ Отправьте КОД в этот бот
