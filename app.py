...
def get_check_photo(msg):
    ...
    for admin in admins:
        try:
            ...
            pass
    bot.send_message(msg.chat.id, f"✅ ЗАЯВКА ПРИНЯТА!\n\n🆔 ID: {account_id}\n💰 СУММА: {amount} сом\n\n⏳ ОЖИДАЙТЕ ОБРАБОТКИ ОПЕРАТОРОМ...", reply_markup=main_menu(user_id))
    del temp_data[user_id]
...
def handle_call(call):
    ...
    if data.startswith('approve_'):
        ...
        if result:
            ...
            try:
                bot.send_message(user_id, f"✅ ВАША ЗАЯВКА НА ПОПОЛНЕНИЕ ОДОБРЕНА!\n\n💰 Сумма: {amount} сом\n🆔 ID: {account_id}\n\nСредства успешно зачислены на ваш игровой счет!")
            except:
                pass
